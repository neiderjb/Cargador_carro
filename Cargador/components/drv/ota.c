...
// Check update task
// downloads every 60sec the json file with the latest firmware
...
void begin_ota() {
	...
	if(Isconnected()){
		ESP_LOGI(TAG,"Connected to wifi network for OTA\n");
		// start the check update task
		xTaskCreate(&check_update_task, "check_update_task", 8192, NULL, 5, NULL);
	}
	else{
		ESP_LOGI(TAG,"NO Connected to wifi network for OTA\n");
	}
}