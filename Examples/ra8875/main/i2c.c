...
#define SPEED 500000				   //100000	50000
...
esp_err_t i2c_FT5206_write(i2c_port_t i2c_num, uint8_t DEVICE_ADDR, uint8_t cmdByte, uint8_t *data, int count)
{
	int ret;
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	...
	i2c_master_stop(cmd);
	ret = i2c_master_cmd_begin(i2c_num, cmd, 10);
	i2c_cmd_link_delete(cmd);
	return ret;
}


esp_err_t i2c_FT5206_read(i2c_port_t i2c_num, uint8_t DEVICE_ADDR, uint8_t DEVICE_REG, uint8_t *data, int count)
{
