...
void SPI_Draw_Pixel(uint16_t color)
{
    uint8_t color_h = (uint8_t)color >> 8;
    uint8_t color_l = (uint8_t)color;
    send8tospi(spi_handle, RA8875_DATAWRITE);
    send8tospi(spi_handle, color_h);
    send8tospi(spi_handle, color_l);
}
