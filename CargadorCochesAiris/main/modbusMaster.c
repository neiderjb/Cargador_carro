...
void sendBit(bool data)
{
    ...
    if ((u16TransmitBufferLength >> 4) < ku8MaxBufferSize)
    {
        ...
        int mask = 1 << txBitIndex;
        _u16TransmitBuffer[_u8TransmitBufferIndex] = (_u16TransmitBuffer[_u8TransmitBufferIndex] & ~mask) | ((data << txBitIndex) & mask);
...
void send32(uint32_t data)
{
    //send(lowWord(data));
    //send(highWord(data));
    send16((uint16_t)data);
    send16((uint16_t)(data >> 16));
}
...
*/
// void preTransmission(void (*preTransmission)(void))
// {
...
*/
// void postTransmission(void (*postTransmission)(void))
// {
...
uint8_t ModbusMasterTransactionOLD(uint8_t u8MBFunction)
{
    ...
    else if (u8MBFunction == ku8MBReadWriteMultipleRegisters)
    {
        u8ModbusADU[u8ModbusADUSize++] = highByte(_u16ReadAddress);
        u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16ReadAddress);
        u8ModbusADU[u8ModbusADUSize++] = highByte(_u16ReadQty);
    ...
    if (u8MBFunction == ku8MBWriteSingleCoil)
    {
        u8ModbusADU[u8ModbusADUSize++] = highByte(_u16WriteAddress);
        u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16WriteAddress);
    }
    ...
    else if (u8MBFunction == ku8MBReadWriteMultipleRegisters)
    {
        ...
        for (i = 0; i < (uint8_t)(_u16WriteQty); i++)
        {
            u8ModbusADU[u8ModbusADUSize++] = highByte(_u16TransmitBuffer[i]);
            u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16TransmitBuffer[i]);
        }
    }
    else if (u8MBFunction == ku8MBMaskWriteRegister)
    {
        u8ModbusADU[u8ModbusADUSize++] = highByte(_u16TransmitBuffer[0]);
        u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16TransmitBuffer[0]);
    ...
    uart_reset_FIFO_ZDU0210RJX(0); //Reset Buffer RX and TX FIFO
    // transmit request
    // if (_preTransmission)
    // {
    //     _preTransmission();
    ...
    for (i = 0; i < u8ModbusADUSize; i++)
    {
        ...
        printf("MODBUS data: %x \n", u8ModbusADU[i]);
        Write_Data_TX_FIFO_ZDU0210RJX(u8ModbusADU[i], 0);
    }

    u8ModbusADUSize = 0;
    //_serial->flush(); // flush transmit buffer
    vTaskDelay(5); //Wait Send all write data
    ...
    u32StartTime = esp_timer_get_time();
    while (u8BytesLeft && !u8MBStatus)
    {
        // if (_serial->available())                                    //Arreglar
        // {
        printf("while ModbusMasterTransaction \n");
        if (Read_Receive_Transmit_FIFO_Level_Registers_ZDU0210RJX(0, 0) > 0)
        {
            //u8ModbusADU[u8ModbusADUSize++] = _serial->read();
            //u8ModbusADU[u8ModbusADUSize++] = Read_Data_RX_FIFO_ZDU0210RJX(0);
            u8BytesLeft--;
            // }
            // else
            // {
        ...
        if (u8ModbusADUSize == 5)
        {
            ...
            else if (u8ModbusADU[1] == ku8MBWriteSingleCoil)
            {
            }
            else if (u8ModbusADU[1] == ku8MBWriteMultipleCoils)
            {
            }
            else if (u8ModbusADU[1] == ku8MBWriteSingleRegister)
            {
            }
            else if (u8ModbusADU[1] == ku8MBWriteMultipleRegisters)
            {
                u8BytesLeft = 3;
            }
            else if (u8ModbusADU[1] == ku8MBMaskWriteRegister)
            {
                u8BytesLeft = 5;
            }
        }

        if ((esp_timer_get_time() - u32StartTime) > ku16MBResponseTimeout)
        {
    ...
    if (!u8MBStatus)
    {
        ...
        else if (u8ModbusADU[1] == ku8MBReadDiscreteInputs)
        {
            ...
            for (i = 0; i < (u8ModbusADU[2] >> 1); i++)
            {
                ...
                {
                    //_u16ResponseBuffer[i] = (uint8_t)(u8ModbusADU[2 * i + 4], u8ModbusADU[2 * i + 3]);
                    _u16ResponseBuffer[i] = (uint16_t)((uint16_t)u8ModbusADU[2 * i + 4] << 8 | u8ModbusADU[2 * i + 3]); //Arreglar
                }
            ...
            if (u8ModbusADU[2] % 2)
            {
                if (i < ku8MaxBufferSize)
                {
                    //_u16ResponseBuffer[i] = (uint16_t)(0, u8ModbusADU[2 * i + 3]);
                    _u16ResponseBuffer[i] = (uint16_t)((uint16_t)0 << 8 | u8ModbusADU[2 * i + 3]);
                }
        ...
        else if (u8ModbusADU[1] == ku8MBReadWriteMultipleRegisters)
        {
            ...
            for (i = 0; i < (u8ModbusADU[2] >> 1); i++)
            {
                if (i < ku8MaxBufferSize)
                {
                    //_u16ResponseBuffer[i] = (uint16_t)(u8ModbusADU[2 * i + 3], u8ModbusADU[2 * i + 4]);
                    _u16ResponseBuffer[i] = (uint16_t)((uint16_t)u8ModbusADU[2 * i + 3] << 8 | u8ModbusADU[2 * i + 4]); //Arreglar
                }
